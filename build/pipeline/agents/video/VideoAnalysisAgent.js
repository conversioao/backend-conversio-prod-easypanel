...
export class VideoAnalysisAgent {
    static async analyze(imageUrl, userPrompt) {
        ...
        try {
            ...
            if (response.usage) {
                await keyManager.logUsage(apiKeyObj.id, 'openai', 'VideoAnalysisAgent', response.usage.prompt_tokens, response.usage.completion_tokens, (response.usage.prompt_tokens * GPT4O_MINI_PRICING.input) + (response.usage.completion_tokens * GPT4O_MINI_PRICING.output));
            }

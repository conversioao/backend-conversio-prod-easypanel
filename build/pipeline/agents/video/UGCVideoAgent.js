...
export class UGCVideoAgent {
    static async generate(params) {
        ...
        try {
            ...
            if (response.usage) {
                await keyManager.logUsage(apiKeyObj.id, 'openai', 'UGCVideoAgent', response.usage.prompt_tokens, response.usage.completion_tokens, (response.usage.prompt_tokens * GPT4O_MINI_PRICING.input) + (response.usage.completion_tokens * GPT4O_MINI_PRICING.output));
            }

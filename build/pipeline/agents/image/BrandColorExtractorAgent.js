...
3. Nunca inventes cores. Lê os tons reais dos píxeis do logótipo.`;
            ...
            if (response.usage) {
                await keyManager.logUsage(apiKeyObj.id, 'openai', 'BrandColorExtractorAgent', response.usage.prompt_tokens, response.usage.completion_tokens, (response.usage.prompt_tokens * GPT4O_MINI_PRICING.input) + (response.usage.completion_tokens * GPT4O_MINI_PRICING.output));
            }

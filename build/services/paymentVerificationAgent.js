...
async function extractDocumentData(fileUrl) {
...
`;
    try {
        ...
        if (isPdf) {
            ...
            const { content } = await processWithOpenAI(systemPrompt, `Analise este texto extraído de um comprovativo bancário em PDF e devolva apenas as informações estruturadas em JSON. Ignore informações irrelevantes.\n\nTEXTO DO PDF:\n${extractedText}`, 'paymentVerificationAgent:pdf', 'gpt-4o-mini', 'json_object');
            ...
            if (fileUrl.toLowerCase().includes('.png'))
                mimeType = 'image/png';
            if (fileUrl.toLowerCase().includes('.webp'))
                mimeType = 'image/webp';
...
async function matchBankData(extracted) {
    const reasons = [];
    try {
        ...
        if (extracted.iban)
            reasons.push(`IBAN detectado no documento: ${extracted.iban}`);
        if (extracted.reference)
            reasons.push(`Referência detectada: ${extracted.reference}`);
...
// ─────────────────────────────────────────────────────────────
async function notifyAdminVerificationResult(tx, extracted, matchResult, verificationStatus) {
    ...
    try {
        ...
        await sendPremiumAdminReport(adminPhone, `COMPROVATIVO ${statusText}`, `Pagamento de ${tx.user_name || 'utilizador'} — ${tx.amount} ${tx.currency}`, isValid
            ? 'Verifique a conta bancária e confirme no painel'
            : 'Analise o comprovativo manualmente — possível fraude', isValid ? 'info' : 'warning');
    }
    catch (e) {
...
// ─────────────────────────────────────────────────────────────
export async function verifyPaymentProof(transactionId) {

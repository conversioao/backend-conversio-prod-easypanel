 ...
 */
export async function qualifyLead(userId) {
    let score = 0;
    try {
        ...
        if (interactions.includes('pricing_viewed'))
            score += 25;
 ...
 */
export function classifyTemperature(score) {
    if (score <= 30)
        return 'cold';
    if (score <= 70)
        return 'warm';
 ...
 */
export async function updateLeadStage(leadId, score) {
    let stage = 'awareness';
    if (score > 30)
        stage = 'interest';
    if (score > 60)
 ...
 */
async function generateFunnelMessage(temperature, name) {
    try {
        let prompt = '';
        if (temperature === 'cold') {
        ...
        const { content: messageBody } = await processWithOpenAI("És um especialista em marketing e vendas de SaaS.", prompt, 'funnelAgent', 'gpt-4o-mini', 'text');
        ...
        if (temperature === 'cold')
            return `Olá ${name}! Passando para partilhar os nossos melhores guias para criar conteúdo que vende todos os dias.`;
        if (temperature === 'warm')
            return `Olá ${name}! Lembras-te de quando falamos sobre multiplicar vendas? Uma parceira nossa triplicou ontem o fluxo usando IA. Vamos testar?`;
 ...
 */
export async function assignNextAction(leadId, userId, temperature) {
    try {
        ...
        if (temperature === 'warm') {
            ...
            await monitor.triggerAlert('hot_lead', 'warning', '🔥 NOVO LEAD QUENTE!', `O utilizador ${userName} (${userId}) atingiu o score HOT. Acção recomendada: Acompanhamento manual urgente.`, { userId, userName });
        }

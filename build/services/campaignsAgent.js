...
async function generateCampaignContent(campaign, user) {
    try {
        ...
        const { content: reply } = await processWithOpenAI("És um copywriter sénior especializado em conversão e retenção de SaaS.", prompt, 'campaignsAgent', 'gpt-4o-mini', 'text');
...
export async function createCampaign(config) {
    const { name, type, target_segment, created_by, message_template } = config;
    const campaign = await query(`
        INSERT INTO campaigns (name, type, target_segment, created_by, message_template, status)
        VALUES ($1, $2, $3, $4, $5, 'active')
        RETURNING id
    `, [name, type, JSON.stringify(target_segment), created_by, message_template]);

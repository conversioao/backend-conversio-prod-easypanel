...
export class KeyManager {
    ...
    async getWorkingKey(provider) {
        try {
            const res = await query(`SELECT id, provider, key_secret, priority, status 
                 FROM api_keys 
     ...
     */
    async reportFailure(keyId, error) {
        try {
            const res = await query(`UPDATE api_keys 
                 SET status = 'failed', is_active = false, last_error = $1, updated_at = NOW() 
                 WHERE id = $2 
                 RETURNING provider, priority`, [error, keyId]);
            if (res.rows.length > 0) {
     ...
     */
    async logUsage(keyId, provider, agentName, tokensPrompt, tokensCompletion, cost) {
        try {
            ...
            await query(`INSERT INTO api_usage_stats (key_id, provider, agent_name, tokens_prompt, tokens_completion, cost_estimated)
                 VALUES ($1, $2, $3, $4, $5, $6)`, [keyId, provider, agentName, tokensPrompt, tokensCompletion, costEstimatedDollar]);
            // 2. Deduct from service balance
            if (provider === 'openai') {
                await query(`UPDATE service_budgets 
                     SET dollar_balance = dollar_balance - $1, 
                         updated_at = NOW() 
                     WHERE service = 'openai'`, [cost]);
            }

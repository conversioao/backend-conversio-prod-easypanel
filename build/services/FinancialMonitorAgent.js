    ...
    try {
        ...
        for (const budget of budgets) {
            ...
            if (consumption >= 0.8) {
                ...
                if (recentAlert.rowCount === 0) {
                    ...
                    await triggerAlert(alertType, 'critical', `💰 ALERTA FINANCEIRO: ${label}`, `O consumo do serviço ${label} atingiu ${pct}%. \nSaldo Restante: ${balance.toFixed(2)}. \n\nAcção Automatizada: Iniciando Backup Preventivo da Base de Dados.`, { service: budget.service, consumption, balance });
        ...
        if (failedKeysRes.rowCount > 0) {
            for (const key of failedKeysRes.rows) {
                await triggerAlert('api_key_failure_detected', 'critical', `🚨 FALHA DE API: ${key.provider.toUpperCase()}`, `A chave "${key.name}" do provedor ${key.provider} falhou recentemente.\nErro: ${key.last_error}`, { provider: key.provider, name: key.name, error: key.last_error });
            }
        }
    }
    catch (error) {

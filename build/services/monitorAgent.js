    ...
    for (const agent of agents.rows) {
        ...
        if (agent.last_run) {
            ...
            if (diffMin > 60) {
                ...
                const pendingTasks = await query(`SELECT COUNT(*) FROM agent_tasks WHERE agent_name = $1 AND status = 'pending'`, [agent.name]);
                const taskCount = parseInt(pendingTasks.rows[0].count);
                if (taskCount > 0) {
                    await triggerAlert('agent_stopped', 'critical', `Agente Travado: ${agent.name}`, `O ${agent.name} tem ${taskCount} tarefas pendentes mas não reporta actividade há ${Math.floor(diffMin)} minutos!`, { agentName: agent.name, lastRun: agent.last_run, pendingTasks: taskCount });
                }
                else {
...
async function checkGenerationFailures() {
    try {
        ...
        for (const gen of failedGens.rows) {
            ...
            await triggerAlert('generation_failed_detailed', severity, title, `ID: ${gen.id}\nErro: ${gen.metadata?.error}\nUser: ${gen.user_id}\nTipo: ${gen.type}`, { generationId: gen.id, error: gen.metadata?.error });
...
async function checkBudgetThresholds() {
    ...
    try {
        ...
        for (const budget of budgets.rows) {
            ...
            let levelToAlert = 0;
            if (consumption >= 80 && !alertedLevels.includes(80))
                levelToAlert = 80;
            else if (consumption >= 70 && !alertedLevels.includes(70))
                levelToAlert = 70;
            if (levelToAlert > 0) {
                ...
                await query(`UPDATE service_budgets SET metadata = metadata || $1 WHERE id = $2`, [JSON.stringify({ alerted_levels: alertedLevels }), budget.id]);
            }
        }
    }
    catch (e) {

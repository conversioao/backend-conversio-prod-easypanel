...
## OUTPUT JSON
{
  ...
  "script_preview": {
    ...
    try {
        ...
        await query('UPDATE prompt_agents SET system_prompt = $1, updated_at = now() WHERE technical_id = $2', [luandaLookPrompt, 'boutique-fashion']);
        console.log('✅ Luanda Look (Image) Prompt upgraded (Strict Portuguese rules).');
        // Update UGC Video
        await query('UPDATE prompt_agents SET system_prompt = $1, updated_at = now() WHERE technical_id = $2', [ugcVideoPrompt, 'ugc-influencer-video']);

...
async function updateSkinToneRules() {
    ...
    try {
        ...
        if (ugcRes.rows[0]) {
            ...
            prompt = prompt.replace(/MODELS\s*•.*Beautiful Angolan women or handsome Angolan men\./s, `MODELS (MANDATORY)
...
• Beautiful Angolan women or handsome Angolan men with Angolan features.`);
        ...
        if (glowRes.rows[0]) {
            ...
            prompt = prompt.replace(/MODELS\s*•.*Beautiful Angolan women and handsome Angolan men/s, `MODELS (MANDATORY)
• ALL MODELS MUST BE BLACK OR BROWN-SKINNED PEOPLE (PESSOAS NEGRAS E MORENAS).
• Naturally textured skin — zero beauty retouching.
• Beautiful Angolan women or handsome Angolan men with Angolan features.`);
        ...
        if (boutiqueRes.rows[0]) {
            ...
            prompt = prompt.replace(/MODELS & REPRESENTATION:.*not smiling generically/s, `MODELS & REPRESENTATION (MANDATORY):
- ALL MODELS MUST BE BLACK OR BROWN-SKINNED PEOPLE (PESSOAS NEGRAS E MORENAS).
